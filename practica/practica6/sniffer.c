...
void procesaPaquete(unsigned char* buffer, int size)
{
    ...
    switch (iph->protocol) //Check the Protocol and do accordingly...
    {
        ...
        case 6:  //TCP Protocol
            ++tcp;
            imprimePaqueteTCP(buffer , size);
            break;
         
        case 17: //UDP Protocol
            ++udp;
            imprimePaqueteUDP(buffer , size);
            break;
...
void imprimeCabeceraIP(unsigned char* Buffer, int Size)
{
...
void imprimePaqueteTCP(unsigned char* Buffer, int Size)
{
    unsigned short iphdrlen;
    ...
    fprintf(logfile,"IP Header\n");
    imprimeDatos(Buffer,iphdrlen);
         
    fprintf(logfile,"TCP Header\n");
    imprimeDatos(Buffer+iphdrlen,tcph->doff*4);
         
    fprintf(logfile,"Data Payload\n");  
    imprimeDatos(Buffer + iphdrlen + tcph->doff*4 , (Size - tcph->doff*4-iph->ihl*4) );
                         
    fprintf(logfile,"\n###########################################################");
}
 
void imprimePaqueteUDP(unsigned char *Buffer , int Size)
{
    ...
    imprimeDatos(Buffer+iphdrlen , sizeof udph);
         
    fprintf(logfile,"Data Payload\n");  
    imprimeDatos(Buffer + iphdrlen + sizeof udph ,( Size - sizeof udph - iph->ihl * 4 ));
     
    fprintf(logfile,"\n###########################################################");
}
 
void imprimePaqueteICMP(unsigned char* Buffer , int Size)
{
    unsigned short iphdrlen;
    ...
    fprintf(logfile,"UDP Header\n");
    imprimeDatos(Buffer + iphdrlen , sizeof icmph);
         
    fprintf(logfile,"Data Payload\n");  
    imprimeDatos(Buffer + iphdrlen + sizeof icmph , (Size - sizeof icmph - iph->ihl * 4));
     
    fprintf(logfile,"\n###########################################################");
}
 
void imprimeDatos (unsigned char* data , int Size)
{
